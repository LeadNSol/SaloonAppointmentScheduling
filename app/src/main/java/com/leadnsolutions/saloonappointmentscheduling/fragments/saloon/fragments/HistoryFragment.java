...
public class HistoryFragment extends Fragment {
    ...
    private void populateData(String tabText) {
        if (SharedPrefHelper.getmHelper().getSaloonModel() != null) {
            SaloonModel model = new Gson().fromJson(SharedPrefHelper.getmHelper().getSaloonModel(), SaloonModel.class);
            DatabaseReference dbRef = FirebaseDatabase.getInstance().getReference(AppConstant.APPOINTMENTS);
            Query query = dbRef.orderByChild("receiverId").equalTo(model.getId());
